...
const Products = () => {
  ...
  const categoryNav = categories.map(category => (
    <motion.div
      key={category.id}
      className={`w-40 p-3 text-center rounded-xl cursor-pointer border border-gray-200 ${
    ...
    >
      <div className="flex flex-col items-center gap-1">
        <img src="/assets/drink.svg" alt={`${category.categoryName} Icon`} className="h-8 w-8" /> {/* Use absolute path */}
        <span className="text-base font-semibold">{category.categoryName}</span>
      </div>
    </motion.div>
  ));
  ...
  return (
    <>
      ...
      <div className="bg-white font-poppins">
        <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20 lg:max-w-7xl lg:px-8">          
          {/* Category Tabs */}
          ...
          <motion.div
            className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4 mb-14"
            initial="hidden"
            animate="visible"
            variants={containerVariants}
          >
